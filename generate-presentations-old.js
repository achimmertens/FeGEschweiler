...
async function createIncomeChart(profitLossReports, years) {
  ...
  const configuration = {
    ...
    options: {
      ...
      // Breitere Balken für bessere Lesbarkeit der Textbeschriftungen
      elements: {
        bar: {
          // Feste Balkendicke in Pixel (wird für wenige Kategorien gut lesbar)
          barThickness: 80,
          // Maximale Balkendicke, falls Chart grösser skaliert
          maxBarThickness: 140
        }
      },
      scales: {
        ...
        y: {
          ...
          ticks: {
            ...
            callback: function(value) {
              // Tausender-Trennpunkt ohne Dezimalstellen
...
async function createExpensesChart(profitLossReports, years) {
  ...
  if (otherCategories.length > 0) {
    ...
    if (anyNonZero) {
      ...
      datasets.push({
        ...
        borderWidth: 1,
        stack: 'stack1',
        // Zeige Beschriftung 'Sonstiges' innerhalb des grauen Bereichs, wenn Wert != 0
        datalabels: {
          display: function(context) {
            try {
              const v = context && context.dataset && context.dataset.data && typeof context.dataIndex === 'number'
          ...
          formatter: function(value, context) {
            try {
              return 'Sonstiges ' + formatGermanInteger(value) + ' €';
            } catch (e) {
              return 'Sonstiges ' + Math.round(value) + ' €';
            }
          }
        }
      });
  ...
  chart.setWidth(1000);
  // Höhe um 80% reduzieren -> 20% der bisherigen Höhe (800 * 0.2 = 160)
  chart.setHeight(700);
  chart.setFormat('png');
  chart.setBackgroundColor('white');
