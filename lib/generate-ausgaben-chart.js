...
async function generateAusgabenChartPlaywright(reportYear, options = {}) {
  ...
  const datasetsForHtml = sortedCats.map((cat, idx) => ({
    label: cat,
    data: years.map(y => { const v = (data[y] && data[y][cat]) || 0; return (cat === 'Schuldenabbau') ? -Math.abs(v || 0) : Math.abs(v || 0); }),
    backgroundColor: (cat === 'Sonstiges') ? '#A9A9A9' : colors[idx % colors.length],
    borderColor: '#ffffff',
    borderWidth: 1,
    stack: 'stack1'
  }));
  ...
  const titlePart = JSON.stringify({ display: true, text: `Ausgaben der letzten 3 Jahre`, font: { size: 30 } }, null, 2);
