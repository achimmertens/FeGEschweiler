...
async function generateAusgabenChartPlaywright(reportYear, options = {}) {
  ...
  // build datasets similar to previous implementation
  ...
  const datasetsForHtml = sortedCats.map((cat, idx) => ({ label: cat, data: years.map(y => { const v = (data[y] && data[y][cat]) || 0; return (cat === 'Darlehensleistungen') ? -Math.abs(v || 0) : Math.abs(v || 0); }), backgroundColor: (cat === 'Sonstiges') ? '#A9A9A9' : colors[idx % colors.length], borderColor: '#ffffff', borderWidth: 1, stack: 'stack1' }));
  ...
  const formatterFn = `function(value, context) { try { var label = (context && context.dataset && context.dataset.label) ? context.dataset.label : ''; var v = (typeof value === 'number') ? value : (value && value.y !== undefined ? value.y : 0); var n = Math.round(Math.abs(Number(v) || 0)); return (label ? label + ': ' : '') + n.toLocaleString('de-DE') + ' €'; } catch (e) { return '' + value; } }`;
  const datalabelsPart = `{"color":"#ffffff","anchor":"center","align":"center","padding":{"left":6},"font":{"weight":"bold","size":16},"formatter":${formatterFn}}`;
  ...
  const outPath = path.join(resultDir, `Ausgaben_${year}.png`);
...
export { generateAusgabenChartPlaywright };
