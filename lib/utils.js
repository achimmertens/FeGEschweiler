...
export function updateEntwicklungCSV() {
  ...
  entwicklungData.forEach(row => { const position = row.Position; let bilanzAccountName = null; for (const [bilanzName, entwicklungName] of Object.entries(accountMapping)) { if (position === entwicklungName) { bilanzAccountName = bilanzName; break; } } if (bilanzAccountName && latestBalances[bilanzAccountName] !== undefined) { let value = latestBalances[bilanzAccountName]; if (position.includes('Darlehen') || position.includes('Privatdarlehen')) value = -Math.abs(value); row[latestYear] = value.toFixed(2).replace('.', ','); } else if (position === 'Summe Aktiva') { const sum = Object.entries(latestBalances).filter(([name]) => name.includes('Girokonto') || name.includes('Sparkonto') || name.includes('Freizeit')).reduce((s, [, v]) => s + v, 0); row[latestYear] = sum.toFixed(2).replace('.', ','); } else if (position === 'Kaution') { const kautionBalance = latestBalances['Kaution']; if (kautionBalance !== undefined) { row[latestYear] = (-Math.abs(kautionBalance)).toFixed(2).replace('.', ','); } } else if (position === 'Summe Passiva' || position === 'Verbindlichkeiten') { const sum = Object.entries(latestBalances).filter(([name]) => ['Darlehenskonto SKB', 'Privatdarlehen', 'Kaution'].includes(name)).reduce((s, [, v]) => s + Math.abs(v), 0); row[latestYear] = (-sum).toFixed(2).replace('.', ','); } else if (position === 'Schuldenabbau') { const prevYear = String(latestYear - 1); const passiva = entwicklungData.find(r => r.Position === 'Summe Passiva' || r.Position === 'Verbindlichkeiten'); if (passiva) { const prevPassiva = Math.abs(parseGermanNumber(passiva[prevYear] || '0')); const currPassiva = Math.abs(parseGermanNumber(passiva[latestYear] || '0')); const schuldenabbau = prevPassiva - currPassiva; row[latestYear] = schuldenabbau.toFixed(2).replace('.', ','); } } });
