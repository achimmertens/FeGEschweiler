...
async function generateEinnahmenChartPlaywright(reportYear, options = {}) {
  ...
  years.forEach(y => { const row = data[y] || {}; Object.entries(row).forEach(([cat, val]) => { categoryTotals[cat] = (categoryTotals[cat] || 0) + Number(val || 0); }); });
  // Remove non-income rows that may appear in the income JSON (e.g., Zinsen für Darlehen)
  if (Object.prototype.hasOwnProperty.call(categoryTotals, 'Zinsen für Darlehen')) delete categoryTotals['Zinsen für Darlehen'];
  ...
  const datasetsForHtml = sortedCats.map((cat, idx) => ({ label: cat, data: years.map(y => Number((data[y] && data[y][cat]) || 0)), backgroundColor: (cat === 'Sonstiges') ? '#A9A9A9' : colors[idx % colors.length], borderColor: '#ffffff', borderWidth: 1, stack: 'stack1' }));

  const dataPart = JSON.stringify({ labels: years, datasets: datasetsForHtml }, null, 2);
  const titlePart = JSON.stringify({ display: true, text: `Einnahmen der letzten 3 Jahre`, font: { size: 30 } }, null, 2);
