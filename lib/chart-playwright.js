...
async function generateAusgabenChartPlaywright(reportYear, options = {}) {
  ...
  const config = {
    ...
    options: {
      indexAxis: 'x',
      plugins: {
        title: { display: true, text: `Ausgaben ${year} nach Kategorien`, font: { size: 20 } },
        legend: { position: 'top' },
        datalabels: {
          color: '#ffffff',
          anchor: 'center',
          ...
          formatter: function(value, context) {
            try {
              ...
              const formattedValue = Number(v).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  ...
  const titlePart = JSON.stringify({ display: true, text: `Ausgaben ${year} nach Kategorien`, font: { size: 20 } }, null, 2);
