...
async function generateAusgabenChartFromJson(reportYear) {
    ...
    const config = {
        ...
        // Build annotations for labels inside segments (QuickChart annotation plugin)
        try {
            const annotations = {};
            // For each year (x index), compute stacked positions
            years.forEach((year, xi) => {
                let posStack = 0; // for positive values
                let negStack = 0; // for negative values (negative numbers)
                datasets.forEach((ds, di) => {
                    const v = ds.data[xi] || 0;
                    const start = v >= 0 ? posStack : negStack;
                    const end = start + v;
                    const center = (start + end) / 2;
                    // Only add annotation if magnitude > threshold
                    if (Math.abs(v) > 0.5) {
                        const parts = Math.abs(v).toFixed(2).split('.');
                        parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, '.');
                        const text = `${ds.label} ${parts.join('.')} €`;
                        const id = `ann_${xi}_${di}`;
                        annotations[id] = {
                            type: 'label',
                            xValue: year,
                            yValue: center,
            ...
            try { fs.appendFileSync(path.join(process.cwd(), 'debug.log'), `[charts] ERROR building annotations: ${e.message}\n`); } catch (ee) {}
        }
