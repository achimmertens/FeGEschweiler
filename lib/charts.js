...
import { readCSV, extractExpenses, ensureResultDirectory, readProfitLossReports, extractIncome, readBalanceReports, extractAccountBalances, COLORS } from './utils.js';
        ...
        try {
            ...
            for (const y of yrs) {
                ...
                Object.entries(bal || {}).forEach(([k,v]) => {
                    ...
                    if (key.includes('darleh') || key.includes('privatdarlehen')) sum += Math.abs(Number(v) || 0);
                });
                if (prevSum === null) {
                    outputData[y]['Schuldenabbau'] = 0;
                } else {
                    const delta = Number((sum - prevSum).toFixed(2));
                    // delta negative when debt decreased -> record as negative
                    outputData[y]['Schuldenabbau'] = delta;
                }
                prevSum = sum;
            }
        } catch (e) {
            try { fs.appendFileSync(path.join(process.cwd(), 'debug.log'), `[charts] Fehler beim Berechnen Schuldenabbau: ${e.message}\n`); } catch (ee) {}
        }
