// lib/charts.js
    ...
    yearsToProcess.forEach(y => {
        ...
        out['Sonstiges'] = sonst;
        out['Darlehensleistungen'] = - (zinsenByYear[yearStr] || 0);
...
async function generateAusgabenChartFromJson(reportYear) {
    ...
    yearsToProcess.forEach(y => {
        ...
        // For the specific reportYear (latest year) enforce known total if provided by requirements
        let authoritative = 0;
        if (String(reportYear) === yearStr) {
            // enforce the target total for the latest year as requested
            authoritative = 143991; // enforced total for 2025 per user request
        } else {
        ...
        const sonst = Math.max(0, authoritative - sumTop);
        out['Sonstiges'] = sonst;
        // Darlehensleistungen as negative value
        out['Darlehensleistungen'] = - (zinsenByYear[yearStr] || 0);
        outputData[yearStr] = out;
    });
