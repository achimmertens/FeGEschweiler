// lib/charts.js
    ...
    // Calculate Schuldenabbau (delta of Darlehenskonto + Privatdarlehen vs previous year)
    try {
        const balanceReports = readBalanceReports();
        // compute sum of debts per year
        const debtSums = {};
        yearsToProcess.forEach(y => {
            const ys = String(y);
            const bal = extractAccountBalances(balanceReports[ys] || []);
            const d1 = Math.abs(bal['Darlehenskonto SKB'] || bal['Darlehenskonto'] || 0);
            const d2 = Math.abs(bal['Privatdarlehen'] || bal['Privatdarlehen 006'] || 0);
            debtSums[ys] = d1 + d2;
        });
        yearsToProcess.forEach((y, idx) => {
            const ys = String(y);
            const prev = idx === 0 ? 0 : (debtSums[String(yearsToProcess[idx - 1])] || 0);
            const curr = debtSums[ys] || 0;
            // Schuldenabbau = current - previous (negative when debt decreased)
            const delta = curr - prev;
            const out = outputData[ys] || {};
            out['Schuldenabbau'] = delta; // keep sign (negative if debt decreased)
            outputData[ys] = out;
        });
    } catch (e) { /* ignore */ }

    // Before saving, compute Schuldenabbau (delta of Darlehenskonto + Privatdarlehen) per year
    try {
        const balanceReports = readBalanceReports();
        const debtSums = {};
        yearsToProcess.forEach(y => {
            const ys = String(y);
            const bal = extractAccountBalances(balanceReports[ys] || []);
            const d1 = Math.abs(bal['Darlehenskonto SKB'] || bal['Darlehenskonto'] || bal['Darlehenskonto SKB 004'] || 0);
            const d2 = Math.abs(bal['Privatdarlehen'] || bal['Privatdarlehen 006'] || 0);
            debtSums[ys] = d1 + d2;
        });
        yearsToProcess.forEach((y, idx) => {
            ...
            const prev = idx === 0 ? 0 : (debtSums[String(yearsToProcess[idx - 1])] || 0);
            const curr = debtSums[ys] || 0;
            const delta = Number((curr - prev).toFixed(2)); // negative when debt decreased
            const out = outputData[ys] || {};
            out['Schuldenabbau'] = delta;
            outputData[ys] = out;
        });
    } catch (e) { /* ignore */ }
...
function generateAusgabenJsonFromSorted(sortedExpensesData, years, currentYear) {
    ...
    // Calculate Schuldenabbau for the sorted-output path: compute (current - previous) of
    // (Darlehenskonto + Privatdarlehen) and store as signed value (negative when debt decreased).
    try {
        const balanceReports = readBalanceReports();
        const debtSums = {};
        yearsToProcess.forEach(y => {
        ...
        yearsToProcess.forEach((y, idx) => {
            ...
            const prev = idx === 0 ? 0 : (debtSums[String(yearsToProcess[idx - 1])] || 0);
            const curr = debtSums[ys] || 0;
            const delta = curr - prev; // negative when debt decreased
            const out = output[ys] || {};
            out['Schuldenabbau'] = delta;
            output[ys] = out;
        });
    ...
    try {
        ...
        yearsToProcess.forEach((y, idx) => {
            ...
            const delta = curr - prev; // negative when debt decreased
            const out = output[ys] || {};
            out['Schuldenabbau'] = Number(delta.toFixed(2));
            output[ys] = out;
        });
    } catch (e) { /* ignore */ }
